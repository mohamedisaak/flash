...
function StatTile({
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg bg-white p-4 shadow-sm">
      <div
        className={`flex h-16 w-16 items-center justify-center rounded-md text-2xl text-white ${color}`}
      >
        {icon}
      </div>
      <div>
  ...
  const tiles =
    admin && statsQuery.data
      ? [
          ...
          {
            label: "Subcategories",
            value: statsQuery.data.subcategories,
            color: "bg-accent",
            icon: "🧭",
          },
          ...
          {
            label: "Online Polls",
            value: statsQuery.data.polls,
            color: "bg-emerald-500",
            icon: "🗳",
          },
          {
            label: "Live Channels",
            value: statsQuery.data.live_channels,
            color: "bg-rose-400",
            icon: "📡",
          },
          {
            label: "Subscribers",
            value: statsQuery.data.subscribers,
            color: "bg-sky-400",
            icon: "📧",
          },
        ]
      : [
          ...
          {
            label: "Published",
            value: posts.filter((a) => a.status === "published").length,
            color: "bg-accent",
            icon: "✅",
          },
          {
            label: "Drafts",
            value: posts.filter((a) => a.status === "draft").length,
            color: "bg-amber-500",
            icon: "📝",
          },
          {
            label: "Scheduled",
            value: posts.filter((a) => a.status === "scheduled").length,
            color: "bg-rose-500",
            icon: "🕒",
          },
        ];
    ...
    <div>
      ...
      <div className="mt-6 rounded-lg bg-white p-6 shadow-sm">
        ...
        <ul className="divide-y divide-[var(--border)]">
          ...
          {posts.length === 0 && (
            <li className="py-2 text-sm text-[var(--muted)]">No posts yet.</li>
          )}
        </ul>
      </div>
    </div>
